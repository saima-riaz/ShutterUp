...
const GalleryDetail = ({ shared = false }) => {
  ...
  const loadGallery = async () => {
    try {
      ...
      if (shared) {
        ...
        const res = await fetch(`${API_BASE}/api/gallery/public/${token}`);
        if (!res.ok) throw new Error("Failed to load shared gallery");
        data = await res.json();
      } else {
        data = await fetchGalleryByUrl(authFetch, url);
      }
  ...
  return (
    <div className="p-8 min-h-screen bg-gradient-to-b from-teal-200 to-blue-300 relative">
      ...
      <div className="mt-8">
        <PhotoGrid
          ...
          shared={shared} // pass shared to disable delete inside PhotoGrid if needed
        />
      </div>
      ...
      {selectedPhoto && (
        <PhotoModal
          ...
          shared={shared} // pass shared if modal has delete button
        />
      )}
    </div>
  );
